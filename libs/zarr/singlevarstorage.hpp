...
class SingleVarStorage {
 protected:
  ...
  void zarrayjsons(const std::string shape, const std::string chunks, const std::string dims) {
 ...
 private:
  ...
  void copy2buffer(const T val) {
    std::tie(ndata, bufferfill) = storehelpers::val2buffer<T>(val, buffer, ndata, bufferfill);
  }

  /* copy values of type T in vector 'vec' to buffer */
  void copy2buffer(const std::vector<T> &vec) {
    std::tie(ndata, bufferfill) = storehelpers::vec2buffer<T>(vec, buffer, ndata, bufferfill);
  }

 public:
  SingleVarStorage(FSStore &store, const unsigned int maxchunk, const std::string name,
  ...
  void is_name(const std::string &goodname) const {
    if (name != goodname) {
      const std::string errmsg("name of storage is " + name + ", but should be " + goodname);
...
#endif  // LIBS_ZARR_SINGLEVARSTORAGE_HPP_
