 ...
 * (see:
 * https://cfconventions.org/Data/cf-conventions/cf-conventions-1.10/cf-conventions.html#_contiguous_ragged_array_representation)
 * in a FFStore obeying zarr storage specification verion 2:
 * https://zarr.readthedocs.io/en/stable/spec/v2.html */

#ifndef LIBS_ZARR_CONTIGRAGGEDSTORAGE_HPP_
#define LIBS_ZARR_CONTIGRAGGEDSTORAGE_HPP_
...
class ContigRaggedStorage {
 private:
  ...
  unsigned int
      ndata;  // number of data points that have been observed (= size of array written to store)
  ...
  void buffers_writejsons() {
    ...
    const SomeMetadata md(zarr_format, order, ndata, chunksize, compressor, fill_value, filters,
                          dims);
  ...
  void buffers_writechunk() {
    std::tie(chunkcount, buffersfill) = buffers.writechunk(store, chunkcount);
  ...
  void copy2buffers(const T &value) {
    std::tie(ndata, buffersfill) = buffers.copy2buffer(value, ndata, buffersfill);
  }

  /* write zarray jsons for array of rgdcount variable in store */
  void rgdcount_writejsons() {
    const std::string count_arrayattrs(
        "{\"_ARRAY_DIMENSIONS\": [\"time\"],"
        "\"sample_dimension\": \"superdroplets\"}");

    const std::string count_metadata =
        storehelpers::metadata(zarr_format, order, rgdcount_ndata, chunksize, rgdcount_dtype,
                               compressor, fill_value, filters);

    storehelpers::writejsons(store, rgdcount_name, count_metadata, count_arrayattrs);
  }
  ...
  void rgdcount_writechunk() {
    std::tie(rgdcount_chunkcount, rgdcount_bufferfill) =
        storehelpers::writebuffer2chunk(store, rgdcount, rgdcount_name, rgdcount_chunkcount);
  ...
  void copy2rgdcount(const size_t raggedn) {
    std::tie(rgdcount_ndata, rgdcount_bufferfill) =
        storehelpers::val2buffer<size_t>(raggedn, rgdcount, rgdcount_ndata, rgdcount_bufferfill);
  }

 public:
  ContigRaggedStorage(FSStore &store, const size_t imaxchunk, const Buffers ibuffers)
      : store(store),
        chunksize(imaxchunk),
        rgdcount(chunksize),
        rgdcount_chunkcount(0),
        rgdcount_bufferfill(0),
        rgdcount_ndata(0),
        buffers(ibuffers),
        chunkcount(0),
        buffersfill(0),
...
#endif  // LIBS_ZARR_CONTIGRAGGEDSTORAGE_HPP_
