...
#ifndef LIBS_ZARR_SUPERDROPSBUFFERS_HPP_
#define LIBS_ZARR_SUPERDROPSBUFFERS_HPP_
...
struct SomeMetadata {
  ...
  SomeMetadata(const char zarr_format, const char order, const unsigned int ndata,
               const size_t chunksize, const std::string compressor, const std::string fill_value,
...
template <typename B>
concept SuperdropsBuffers =
    requires(B b, Superdrop &superdrop, FSStore &store, std::string &str, unsigned int u,
             SomeMetadata &md) {
      { b.copy2buffer(superdrop, u, u) } -> std::same_as<std::pair<unsigned int, unsigned int>>;

      { b.writechunk(store, u) } -> std::same_as<std::pair<unsigned int, unsigned int>>;
...
struct CombinedSuperdropsBuffers {
  ...
  CombinedSuperdropsBuffers(A a, B b) : a(a), b(b) {}
  ...
  std::pair<unsigned int, unsigned int> writechunk(FSStore &store, const unsigned int chunkcount) {
...
struct NullSuperdropsBuffer {
  ...
  std::pair<unsigned int, unsigned int> writechunk(FSStore &store,
                                                   const unsigned int chunkcount) const {
...
#endif  // LIBS_ZARR_SUPERDROPSBUFFERS_HPP_
