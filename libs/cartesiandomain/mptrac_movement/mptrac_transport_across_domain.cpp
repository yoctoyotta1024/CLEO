...
viewd_supers totsupers) const
{
  ...
  for (size_t ip = 0; ip < nparticles; ++ip){
    ...
    int rank_dest = comm_rank; // default is indicator to MPTRAC that particles stay on current rank
  ...
  if (comm_rank == is_first_rank) {
    for (size_t ip = 0; ip < nparticles; ++ip){
      ...
      if (drop.sdId.get_value() == is_first_sdId){
          ...
          std::cout << "first superdrop that moved: q[1]=" << drop.get_coord1() << "\n";
          std::cout << "first superdrop that moved: q[2]=" << drop.get_coord2() << "\n";
          std::cout << "first superdrop that moved: q[3]=" << drop.get_coord3() << "\n";
  ...
  const int nquantities) const
{
  ...
  MPI_Aint displacements[nblocks] = { offsetof (particle_t, q) };
