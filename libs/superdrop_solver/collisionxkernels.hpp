...
public:
  ...
  bueff = 1 - coaleff */
  {
    ...
    constexpr double energylim = 5e-6; // etot limit [J]

    const double cke(ck.collision_kinetic_energy(drop1, drop2));
    const double surf_t(ck.total_surfenergy(drop1, drop2)); // [J] S_t
    const double surf_c(ck.coal_surfenergy(drop1, drop2));  // [J] S_c
