...
struct Config {
 private:
  ...
  void open_file(const std::string_view config_filename, std::ifstream &file) const;
  ...
  void configvariable_thermosolverfromfile(const std::string name, const std::string value);

  /* setter function for assigning 'value' to members of
  Config struct called 'name' specifically for members
  involved when thermosolver == 'cvode' */
  void configvariable_thermosolvercvode(const std::string name, const std::string value);
 ...
 public:
  ...
  double TEMP_INIT =
      std::numeric_limits<double>::signaling_NaN();  // initial parcel temperature [T]
  double relh_init = std::numeric_limits<double>::signaling_NaN();  // initial relative humidity (%)

  /* ODE parameters */
  double W_AVG =
      std::numeric_limits<double>::signaling_NaN();  // average amplitude of sinusoidal vertical
                                                     // parcel speed [m/s] (dP/dt ~ w*dP/dz)
  double T_HALF = std::numeric_limits<double>::signaling_NaN();      // timescale for w sinusoid,
                                                                     // tau_half = T_HALF/pi [s]
  double cvode_rtol = std::numeric_limits<double>::signaling_NaN();  // relative tolerance for [P,
                                                                     // T, qv, qc] ODEs integration
  double cvode_atol = std::numeric_limits<double>::signaling_NaN();  // absolute tolerances for [P,
                                                                     // T, qv, qc] ODEs integration
