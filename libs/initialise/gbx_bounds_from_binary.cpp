...
GbxBoundsFromBinary::GbxBoundsFromBinary(const size_t ngbxs, const unsigned int nspacedims,
  ...
  gbxindexs = vector_from_binary<unsigned int>(file, meta.at(1));
  gbxbounds = vector_from_binary<double>(file, meta.at(2));

  file.close();

  if (6 * gbxindexs.size() != gbxbounds.size() && gbxbounds.size() >= 6) {
    std::string errormsg =
        "sizes of gbxindexs and gbxbounds vectors"
...
gbxindexs vector to position where gbxindex matches idx
(ie. *it = idx) */
size_t GbxBoundsFromBinary::find_idx_in_gbxindexs(const unsigned int idx) const {
  auto it(std::find(gbxindexs.begin(), gbxindexs.end(), idx));  // iterator to idx
  size_t pos(std::distance(gbxindexs.begin(), it));  // distance from start of gbxindexs to idx

  if (pos > (gbxindexs.size() - 1)) {
    /* if pos is larger than the largest valid position
    in gbxindexs, idx has not been found so throw an error*/
    throw std::invalid_argument("idx not found in gbxindexs");
  }
...
Kokkos::pair<double, double> GbxBoundsFromBinary::get_coord3gbxbounds(
  ...
  const unsigned int pos = find_idx_in_gbxindexs(idx) * 6;  // position of zmin for gbxindex = idx
