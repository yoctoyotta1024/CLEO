...
static double dp_dt(const double t,
                    const double wmax,
  ...
  note: true dP/dt = dp/dt * P0/TIME0 */
{
  ...
  // double w(wmax * 2/ M_PI);                                 // non-sinusoidal, constant velocity profile
  // double z(w * t);                                          // linear z coordinate
  ...
  note: True dTemp/dt = dtemp * TEMP0/TIME0  */
{
  ...
  double rho_d(dlc::Mr_ratio / (dlc::Mr_ratio + qv) * p / temp); // density of dry parcel (p_dry/temp)
