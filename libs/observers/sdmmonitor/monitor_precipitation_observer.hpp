 ...
 * the bottom boundary of each gridbox, i.e. output = downward mass flux of water / water density
 */
 ...
 * flux of water / water density) to an array with a constant observation timestep "interval".
 *
...
inline Observer auto MonitorPrecipitationObserver(const unsigned int interval, Dataset& dataset,
  ...
  const auto units = std::string_view("mm/hr");
  constexpr auto scale_factor = dlc::COORD0 * 1000.0 / dlc::TIME0 * 3600.0;
