...
struct ImplicitIteration {
  ...
  KOKKOS_INLINE_FUNCTION double initialguess(const double rprev) const;
  ...
  KOKKOS_INLINE_FUNCTION Kokkos::pair<bool, double> iterate_rootfinding_algorithm(
...
Refer to section 5.1.2 Shima et al. 2009 and section 3.3.3 of
Matsushima et al. 2023 for more details. */
KOKKOS_INLINE_FUNCTION double ImplicitEuler::solve_condensation(
...
5.1.2 Shima et al. 2009 and section 3.3.3 of Matsushima et al. 2023
for more details. */
KOKKOS_INLINE_FUNCTION double ImplicitEuler::solve_condensation_matsushima(
    const double s_ratio, const Kokkos::pair<double, double> kohler_ab, const double ffactor,
    const double rprev) const {
  const auto akoh = double{kohler_ab.first};
  const auto bkoh = double{kohler_ab.second};
...
KOKKOS_INLINE_FUNCTION double ImplicitEuler::substepped_implicitmethod(
...
much greater than its (activation radius)^2 if the
supersaturation > its activation supersaturation  */
KOKKOS_INLINE_FUNCTION double ImplicitIteration::initialguess(const double rprev) const {
...
Second criteria is that initial guess >= 'r1sqrd', where r1 is the
equilibrium radius of a given droplet when s_ratio=1  */
KOKKOS_INLINE_FUNCTION double ImplicitIteration::initialguess_shima(const double rprev) const {
...
'niters' iterations, convergence criteria is tested and futher
iterations undertaken if not yet converged. */
KOKKOS_INLINE_FUNCTION double ImplicitIteration::newtonraphson_niterations(const double rprev,
                                                                           double ziter) const {
...
equations from "An Introduction To Clouds...."
(see note at top of file). Note: z = ziter = radius^2 */
KOKKOS_INLINE_FUNCTION double ImplicitIteration::ode_gfunc(const double rprev,
...
respect to z=rsqrd. g(z) is polynomial to find root of using
Newton Raphson Method. */
KOKKOS_INLINE_FUNCTION double ImplicitIteration::ode_gfuncderivative(const double rsqrd) const {
  const auto radius = double{Kokkos::sqrt(rsqrd)};
...
(which is the radius at timestep 't+subdelt'. Refer to section 5.1.2 Shima
et al. 2009 and section 3.3.3 of Matsushima et al. 2023 for more details. */
KOKKOS_INLINE_FUNCTION double ImplicitIteration::newtonraphson_untilconverged(
