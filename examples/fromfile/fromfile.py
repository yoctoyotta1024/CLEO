...
config_filename = path2build / "tmp" / "fromfile_config.yaml"
thermofiles = sharepath / "fromfile_dimlessthermo.dat"
config_params = {
    "constants_filename": str(path2CLEO / "libs" / "cleoconstants.hpp"),
    "grid_filename": str(sharepath / "fromfile_dimlessGBxboundaries.dat"),
    "initsupers_filename": str(sharepath / "fromfile_dimlessSDsinit.dat"),
    "setup_filename": str(binpath / "fromfile_setup.txt"),
    "zarrbasedir": str(binpath / "fromfile_sol.zarr"),
}
...
def plot_results(path2CLEO, config_filename, savefigpath):
    plotting_script = path2CLEO / "examples" / "fromfile" / "fromfile_plotting.py"
    python = sys.executable
    ...
    # equivalent to ``import fromfile_plotting`` followed by
    # ``fromfile_plotting.main(path2CLEO, savefigpath, ...)``
    cmd = [
        python,
        plotting_script,
