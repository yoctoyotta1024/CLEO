...
def parse_known_arguments():
    ...
    parser.add_argument(
        ...
        default="/work/mh0731/m300950/icon-mpim/experiments/bubble_cleo/bin/plots",
    )
    parser.add_argument(
        "--grid_filename",
        type=Path,
        help="Absolute path to gridbox boundaries file",
        default="/work/mh0731/m300950/icon-mpim/experiments/bubble_cleo/share/bubble3d_dimlessGBxboundaries.dat",
    )
    parser.add_argument(
        "--setupfile",
        type=Path,
        help="Absolute path to setup file",
        default="/work/mh0731/m300950/icon-mpim/experiments/bubble_cleo/bin/bubble3d_setup.txt",
    )
    parser.add_argument(
        "--dataset",
        type=Path,
        help="Absolute path to dataset",
        default="/work/mh0731/m300950/icon-mpim/experiments/bubble_cleo/bin/bubble3d_sol.zarr",
    )
...
def plot_2d_var(
    ...
    fig.suptitle("Cleo output from cleo_bubble")
...
def plot_2d_winds(
    ...
    for m in range(0, nplots):
        ...
        title = "t={:.0f}mins".format(time.mins[tidx])
        if m == 0:
            title = f"{var}\n" + title
        axs[m].set_title(title, fontsize=10)
        axs[m].set_xlabel("x /km")
        axs[m].sharey(axs[0])
        axs[m].sharex(axs[0])
    ...
    fig.suptitle("Cleo output of winds from cleo_bubble")
...
def plot_2d_var_zymidplane(
    ...
    fig.suptitle("Z-Y Cross-section of Cleo output from cleo_bubble")

    return fig, axs


def plot_2d_supers(xxh_km, zzh_km, wind_var, label, t2plts, sample, cmap, vlims, xlims):
    nplots = len(t2plts)
...
# savename = savefigpath / "bubble_winds.png"
